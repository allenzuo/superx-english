...
onLoad((options: any) => {
  if (options.type) {
    readingType.value = options.type
    loadArticlesByType(options.type)
  }
})

async function loadArticles() {
  let type = readingType.value || 'standard'
  await loadArticlesByType(type)
}

async function loadArticlesByType(type: string) {
  let data: any[] = []
  let typeName = '阅读理解'
  ...
  if (data && Array.isArray(data)) {
    articles.value = data.map((item: any, index: number) => ({
      ...
      summary: item.passage?.substring(0, 100) || item.content?.substring(0, 100) || item.article?.substring(0, 100) || item.description?.substring(0, 100) || '',
      type: typeName,
      difficulty: getDifficulty(item.difficulty),
      wordCount: item.wordCount || item.totalWordCount || Math.floor((item.passage?.length || item.content?.length || item.article?.length || 0) / 6),
